...
This is the intuition behind our paper, written with Philip Zmushko and Peter Richtárik. This project page builds the method from the adapter view, explains the counterintuitive TopK-versus-BottomK choice, and puts the results in their proper experimental context.
...
<figure>
  <Image
    ...
    loading="lazy"
  />
  <figcaption>Pruning asks what to remove; sparse fine-tuning asks what to train. The shared problem is mask selection. Slide 18 from the project presentation.</figcaption>
</figure>
...
Super—short for **Selective Update of Parameters via Extreme Ranking**—reuses this score, not the full Wanda pruning procedure. It ranks all coordinates within each adapted layer, while many pruning implementations impose row-wise budgets. A calibration pass through the frozen model supplies the activation norms; it does not require fine-tuning gradients. In our reported setup, the default mask construction uses 128 C4 calibration samples.
...
The presentation writes the squared Wanda score, $W_{ij}^{2}\lVert X_{j:}\rVert_2^{2}$. Squaring does not change the ordering because the scores are nonnegative.
...
<figure>
  <Image
    ...
    loading="lazy"
  />
  <figcaption>The BottomK direction was a hypothesis to test, not a theorem. Slide 26 uses the squared score, which produces the same ordering as the formula above.</figcaption>
</figure>
...
We also tested a magnitude-only ordering, $|W_{ij}|$, in the spirit of PaFi. It needs no activation calibration at all—and, especially on the 8B model, it turned out to be a remarkably strong baseline.
...
<figure>
  <Image
    ...
    loading="lazy"
  />
  <figcaption>Supra combines a fixed coordinate-level update with an adaptive low-rank update. Slide 24 from the project presentation.</figcaption>
</figure>
...
All main comparisons use $r_0=8$. The point is not that the implementations have identical runtime; it is that they are compared with approximately the same number of trainable and stored adapter scalars.
...
<figure>
  <Image
    ...
    loading="lazy"
  />
  <figcaption>Training and validation losses show that the adapters optimize successfully, but exact-answer accuracy is the metric that determines whether the generated result is correct. Slide 32.</figcaption>
</figure>
...
<figure>
  <Image
    ...
    loading="lazy"
  />
  <figcaption>Observed single-seed, schedule-selected averages for Llama-3.2-1B. Adapter methods use the rank-8-equivalent budget; full fine-tuning is an unbudgeted reference. Slide 38.</figcaption>
</figure>
...
<figure>
  <Image
    ...
    loading="lazy"
  />
  <figcaption>Observed single-seed, schedule-selected averages for Meta-Llama-3-8B. Supra-Mag and magnitude-only BottomK are effectively tied. Slide 42.</figcaption>
</figure>
...
There are four qualifications that matter when reading these charts.
...
4. **The reported full-snapshot benchmark protocol is not held out from Math17K.** Math17K includes questions drawn from the first 80% of the six packaged benchmark snapshots. The repository now documents this explicitly and includes `supertuning-data-audit` plus disjoint benchmark subsets. The chart values should therefore be read as results for the paper's reproducible packaged protocol, not as unseen-question generalization estimates.

<aside class="project-note">
  <strong>Why keep the last point prominent?</strong> A useful method claim and a clean evaluation claim are different things. The mask-selection comparison remains reproducible, but future evidence should emphasize disjoint questions, multiple seeds, and more task families.
</aside>
